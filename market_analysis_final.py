...
# A股板块ETF
ETF_CODES = ['512480.SS', '512800.SS', '512690.SS', '512010.SS', '516160.SS']
ETF_NAMES = {'512480.SS': '半导体ETF', '512800.SS': '银行ETF', 
             '512690.SS': '酒ETF', '512010.SS': '医药ETF', '516160.SS': '新能源ETF'}

# 合并所有板块
SECTORS = {}
for code in ETF_CODES:
    SECTORS[code] = ETF_NAMES.get(code, code)

PERIODS = {'本周': 5, '本月': 20, '本季': 60}

# ============ 数据获取 ============
ALL_TICKERS = PRECIOUS_METALS + list(INDICES) + ETF_CODES
...
for period, days in [('本周', 5), ('本月', 20), ('本季', 60)]:
    ...
    for ticker in ETF_CODES:
        if ticker in close_data.columns:
            ...
            if ret is not None:
                results.append((ETF_NAMES.get(ticker, ticker), ret))
...
report.append("\nA股操作:")
semi_ret = get_returns(close_data.get('512480.SS'), 20) if '512480.SS' in close_data.columns else None
bank_ret = get_returns(close_data.get('512800.SS'), 20) if '512800.SS' in close_data.columns else None
wine_ret = get_returns(close_data.get('512690.SS'), 20) if '512690.SS' in close_data.columns else None
newenergy_ret = get_returns(close_data.get('516160.SS'), 20) if '516160.SS' in close_data.columns else None

if semi_ret and semi_ret > 5:
    report.append("  - 半导体ETF: 涨幅较大，谨慎追高")
elif semi_ret and semi_ret > 0:
    report.append("  - 半导体ETF: 温和上涨，AI芯片龙头关注")
else:
    report.append("  - 半导体ETF: 回调后布局，关注先进封装")

if newenergy_ret and newenergy_ret > 0:
    report.append(f"  - 新能源ETF: 本月+{newenergy_ret:.2f}%，趋势向上")
elif newenergy_ret:
    report.append("  - 新能源ETF: 回调后逢低布局")

if bank_ret and bank_ret > 0:
    report.append(f"  - 银行ETF: 本月{bank_ret:+.2f}%，防御配置")
elif bank_ret:
    report.append("  - 银行ETF: 高股息，适合长期配置")
