...
INDICES = ['000001.SS', '399001.SZ', '399006.SZ', '000688.SS', '000300.SS']
INDEX_NAMES = {'000001.SS': '上证指数', '399001.SZ': '深证成指', 
               '399006.SZ': '创业板指', '000688.SS': '科创50', '000300.SS': '沪深300'}

SECTORS = ['399811.SZ', '399986.SZ', '399997.SZ', '399300.SZ', '000941.SS', '931071.CS']
SECTOR_NAMES = {'399811.SZ': '中证半导体', '399986.SZ': '中证银行', '399997.SZ': '中证白酒',
                '399300.SZ': '中证医药', '000941.SS': '新能源', '931071.CS': '中证AI'}

PERIODS = {'本周': 5, '本月': 20, '本季': 60}

# ============ 数据获取 ============
ALL_TICKERS = PRECIOUS_METALS + INDICES + SECTORS
print(f"\n获取 {len(ALL_TICKERS)} 个标的数据...")
...
for ticker in PRECIOUS_METALS:
    ...
    report.append(f"\n{name}:")
    report.append(f"  最新价: {latest}")
    report.append(f"  本周: {w_ret:+.2f}%" if w_ret else "  本周: N/A")
    report.append(f"  本月: {m_ret:+.2f}%" if m_ret else "  本月: N/A")
    report.append(f"  本季: {q_ret:+.2f}%" if q_ret else "  本季: N/A")
...
for period, days in [('本周', 5), ('本月', 20), ('本季', 60)]:
    ...
    if results:
        ...
        for i, (name, ret) in enumerate([r for r in results if r[1] > 0][:3], 1):
            report.append(f"    {i}. {name} +{ret:.2f}%")
        
        # 领跌
        report.append("  领跌TOP3:")
        neg_results = [r for r in results if r[1] < 0]
        for i, (name, ret) in enumerate(neg_results[:3], 1):
            report.append(f"    {i}. {name} {ret:.2f}%")
